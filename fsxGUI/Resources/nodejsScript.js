...
(() => {
    ...
    class Utils {
        constructor() { throw new Error("Don't instantiate me!"); }
        static Wait(ms) {
    ...
    ;// CONCATENATED MODULE: ./node_modules/@lucas-bortoli/libdiscord-fs/lib/webhook.js

    const { https: webhook_https } = follow_redirects;
            ...
            while (!msg) {
                try {
                    const body = await new Promise((resolve, reject) => {
                        ...
                        const req = webhook_https.request({
                            protocol: url.protocol,
                            hostname: url.hostname,
                            path: url.pathname,
                            port: url.port,
    ...
    ;// CONCATENATED MODULE: ./node_modules/@lucas-bortoli/libdiscord-fs/lib/streams.js


    const BLOCK_SIZE = Math.floor(7.6 * 1024 * 1024);
    class RemoteWriteStream extends external_stream_.Writable {
        constructor(webhook) {
    ...
    class RemoteReadStream extends external_stream_.Readable {
    ...
    ;// CONCATENATED MODULE: ./node_modules/@lucas-bortoli/libdiscord-fs/lib/filesystem.js
    ...
    class Filesystem {
        ...
        async loadDataFromStream(stream) {
        ...
        async createReadStream(filePath) {
            ...
            const pieces = new external_util_.TextDecoder('utf-8').decode(piecesBlob).split(',');
         ...
         */
        async createWriteStream(filePath, createEntry = true, customEntryProperties = {}) {
        ...
        mv(from, to) {
            if (to.endsWith('/'))
                to += posix_namespaceObject.basename(from);
        ...
        cp(fromPath, toPath) {
            if (toPath.endsWith('/'))
                toPath += posix_namespaceObject.basename(fromPath);
         ...
         */
        async walkDirectory(root, cb, _prevPath = '/') {
            for (const [name, entry] of Object.entries(root.items)) {
                if (entry.type === 'directory') {
                    await this.walkDirectory(entry, cb, posix_namespaceObject.join(_prevPath, name));
                }
                else {
                    await cb(entry, posix_namespaceObject.join(_prevPath, name));
                }
            }
        }
        /**
...
/* harmony default export */ const lib = (Filesystem);

    ;// CONCATENATED MODULE: external "fs/promises"
    const promises_namespaceObject = __WEBPACK_EXTERNAL_createRequire(import.meta.url)("fs/promises");
    ;// CONCATENATED MODULE: ./dist_tsc/utils.js


    const REMOTE_PATH_WITH_DRIVE_REGEXP = /^(.*[^\/])::(\/.*)$/m;
    const REMOTE_PATH_WITHOUT_DRIVE_REGEXP = /^(\/.*)$/;
    class utils_Utils {
        static naturalSort(entries) {
         ...
         */
        static async fsp_fileExists(path) {
            try {
                await promises_namespaceObject.access(path, external_fs_.constants.F_OK);
                return true;
            }
            catch (error) {
                return false;
    ...
    ;// CONCATENATED MODULE: ./dist_tsc/index.js
    ...
    const openFileSystem = async () => {
        if (fileSystem)
            return fileSystem;
        const libfs = new lib(process.env.DISCORD_WEBHOOK);
        fileSystem = libfs;
        let dataFile = external_path_.resolve(process.env.FSX_DRIVE);
        // If file doesn't exist, proceed with an empty filesystem
        if (!await utils_Utils.fsp_fileExists(dataFile)) {
            console.log('Filesystem doesn\'t exist, proceeding with an empty drive. Given path: ' + dataFile);
            return libfs;
        }
        const fileStream = external_fs_.createReadStream(dataFile);
        await libfs.loadDataFromStream(fileStream);
        console.log('Loaded filesystem from disk.');
        return libfs;
    };
    const saveFileSystem = async (fsx) => {
        const fileStream = external_fs_.createWriteStream(process.env.FSX_DRIVE);
    ...
    const server = external_http_.createServer(async (req, res) => {
        ...
        const doResponse = (output) => {
            console.log(output);
            res.writeHead(200);
            res.end(output.map(row => row.map(cell => encodeURIComponent(cell)).join(',')).join('\n'));
        };
        ...
        if (operation === 'download') {
            ...
            if (!utils_Utils.isValidRemotePath(operand1))
                return doError(`download: Invalid remote path ${operand1} (not in format driveId::/path/to/file)`);
            const { driveId, remotePath } = utils_Utils.parseRemotePath(operand1);
        ...
        if (operation === 'mv') {
            ...
            if (!utils_Utils.isValidRemotePath(operand2))
                return doError(`mv: Invalid remote path ${operand2} (not in format driveId::/path/to/file)`);
            const pathFrom = utils_Utils.parseRemotePath(operand1);
            const pathTo = utils_Utils.parseRemotePath(operand2);
        ...
        if (operation === 'rm') {
            ...
            const target = utils_Utils.parseRemotePath(operand1);
            const fileSystem = await openFileSystem();
        ...
        if (operation === 'ls') {
            ...
            const path = utils_Utils.parseRemotePath(operand1);
            const fileSystem = await openFileSystem();
            ...
            const response = [];
            for (const [name, child] of utils_Utils.naturalSort(Object.entries(target.items))) {
                if (child.type === 'directory') {
                    const howManySubitems = Object.values(child.items).length;
        ...
        if (operation === 'get-dir-tree') {
            ...
            const { remotePath } = utils_Utils.parseRemotePath(operand1);
            const fileSystem = await openFileSystem();
            ...
            const response = [];
            const getDirTree = (root, _prevPath) => {
                _prevPath = _prevPath || '/';
                let paths = [];
                for (const [name, entry] of Object.entries(root.items)) {
                    if (entry.type === 'directory') {
                        let dir = entry;
                        let thisPath = posix_namespaceObject.join(_prevPath, name);
